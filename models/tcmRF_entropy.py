...
class RefTCM(CompressionModel):
    def __init__(self, config=[2, 2, 2, 2, 2, 2], head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0, N=128, M=320, num_slices=5, max_support_slices=5, num_refs=3, **kwargs):
    ...
    def forward(self, x, refs):
        y = self.g_a(x)
        y_refs = [self.g_a(ref) for ref in refs]
        
        # 融合参考图像特征
        y_fused = self.ref_fusion(torch.cat([y] + y_refs, dim=1))
        
        y_shape = y_fused.shape[2:]
        z = self.h_a(y_fused)
        ...
        y_slices = y_fused.chunk(self.num_slices, 1)
        y_hat_slices = []
        y_likelihood = []
        mu_list = []
        scale_list = []
        ...
        return {
            ...
            "para":{"means": means, "scales":scales, "y":y_fused}
        }

    def compress(self, x, refs):
        y = self.g_a(x)
        y_refs = [self.g_a(ref) for ref in refs]
        y_fused = self.ref_fusion(torch.cat([y] + y_refs, dim=1))
    ...
    def decompress(self, strings, shape, refs):
        y_refs = [self.g_a(ref) for ref in refs]
        ...
        # 融合参考图像特征
        y_fused = self.ref_fusion(torch.cat([y_hat] + y_refs, dim=1))
        
        x_hat = self.g_s(y_fused).clamp_(0, 1)
