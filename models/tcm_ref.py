...
class Patch_Matching(nn.Module):
    def __init__(self, dim) -> None:
        super().__init__()
        self.conv1 = conv(dim, dim, 5, 1)
        self.conv2 = conv(dim, dim, 5, 1)

    def forward(self, x, ref):
        x = self.conv1(x)
        ref = self.conv2(ref)
        ref = torch.cat((x, ref), dim=1)
...
class TCM(CompressionModel):
    def __init__(self, config=[2, 2, 2, 2, 2, 2], head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0, N=128,  M=320, num_slices=5, max_support_slices=5, **kwargs):
        ...
        self.g_s = nn.Sequential(*[ResidualBlockUpsample(M*2, 2*N, 2)] + self.m_up1 + self.m_up2 + self.m_up3)
        ...
        self.patch_matching = Patch_Matching(self.M)
        self.align = Align(self.M * 2)
    ...
    def forward(self, x, ref_x):
        ...
        #ref_x
        ref_y = self.g_a(ref_x)
        ref_y_shape = ref_y.shape[2:]
        ref_z = self.h_a(ref_y)
        _, ref_z_likelihoods = self.entropy_bottleneck(ref_z)

        ref_z_offset = self.entropy_bottleneck._get_medians()
        ref_z_tmp = ref_z - ref_z_offset
        ref_z_hat = ste_round(ref_z_tmp) + ref_z_offset

        ref_latent_scales = self.h_scale_s(ref_z_hat)
        ref_latent_means = self.h_mean_s(ref_z_hat)

        ref_y_slices = y.chunk(self.num_slices, 1)
        ref_y_hat_slices = []
        ref_y_likelihood = []
        ref_mu_list = []
        ref_scale_list = []
        for slice_index, y_slice in enumerate(ref_y_slices):
            ref_support_slices = (ref_y_hat_slices if self.max_support_slices < 0 else ref_y_hat_slices[:self.max_support_slices])
            ref_mean_support = torch.cat([ref_latent_means] + ref_support_slices, dim=1)
            ref_mean_support = self.atten_mean[slice_index](ref_mean_support)
            ref_mu = self.cc_mean_transforms[slice_index](ref_mean_support)
            ref_mu = ref_mu[:, :, :ref_y_shape[0], :ref_y_shape[1]]
            ref_mu_list.append(ref_mu)
            ref_scale_support = torch.cat([ref_latent_scales] + ref_support_slices, dim=1)
            ref_scale_support = self.atten_scale[slice_index](ref_scale_support)
            ref_scale = self.cc_scale_transforms[slice_index](ref_scale_support)
            ref_scale = ref_scale[:, :, :ref_y_shape[0], :ref_y_shape[1]]
            ref_scale_list.append(ref_scale)
            _, ref_y_slice_likelihood = self.gaussian_conditional(y_slice, ref_scale, ref_mu)
            ref_y_likelihood.append(ref_y_slice_likelihood)
            ref_y_hat_slice = ste_round(y_slice - ref_mu) + ref_mu
            # if self.training:
            #     lrp_support = torch.cat([mean_support + torch.randn(mean_support.size()).cuda().mul(scale_support), y_hat_slice], dim=1)
            # else:
            ref_lrp_support = torch.cat([ref_mean_support, ref_y_hat_slice], dim=1)
            ref_lrp = self.lrp_transforms[slice_index](ref_lrp_support)
            ref_lrp = 0.5 * torch.tanh(ref_lrp)
            ref_y_hat_slice += ref_lrp

            ref_y_hat_slices.append(ref_y_hat_slice)

        ref_feature = self.feature_extractor(ref_x)


        y_hat = torch.cat(y_hat_slices, dim=1)
        ref_y_hat = torch.cat(ref_y_hat_slices, dim=1)
        ref_y_hat = self.patch_matching(y_hat, ref_y_hat)
        ref_y_hat = self.align(ref_feature, ref_y_hat)
        y_hat = torch.cat((y_hat, ref_y_hat), dim=1)
        x_hat = self.g_s(y_hat)

        #计算损失时需要考虑reference图片的对应结果，当前只有目标图片的结果
        means = torch.cat(mu_list, dim=1)
        scales = torch.cat(scale_list, dim=1)
        y_likelihoods = torch.cat(y_likelihood, dim=1)

        return {
            "x_hat": x_hat, #MSE loss
            "likelihoods": {"y": y_likelihoods, "z": z_likelihoods}, #BPP loss
