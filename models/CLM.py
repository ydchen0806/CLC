...
NOTE: Uses GroupNorm instead of BatchNorm to avoid train/eval statistics mismatch.
      BN uses batch stats during training but running stats during inference (compress),
      causing the CLM to produce different y_a for the same input. GroupNorm is
      consistent across train/eval because it normalizes per-sample.
"""
...
class FeatureTransform(nn.Module):
    """φ(·): 3×3 CNN with GroupNorm+ReLU.
    GroupNorm (not BatchNorm) ensures train/eval consistency."""
    def __init__(self, in_ch, out_ch=512, num_groups=32):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, 3, padding=1),
            nn.GroupNorm(num_groups, out_ch), nn.ReLU(True),
...
class MultiScaleDeformAlign(nn.Module):
    ...
    All normalization uses GroupNorm for train/eval consistency."""
    def __init__(self, ch, num_groups=32):
        super().__init__()
        g = min(num_groups, ch)  # safety for small ch
        # 5×5 coarse
        self.off5 = nn.Sequential(nn.Conv2d(ch*2, ch, 3, padding=1), nn.GroupNorm(g, ch), nn.GELU(),
                                  nn.Conv2d(ch, 2*25, 3, padding=1))
        self.mod5 = nn.Sequential(nn.Conv2d(ch*2, ch, 3, padding=1), nn.GroupNorm(g, ch), nn.GELU(),
                                  nn.Conv2d(ch, 25, 3, padding=1))
        self.w5 = nn.Parameter(torch.empty(ch, ch, 5, 5)); nn.init.kaiming_normal_(self.w5)
        self.b5 = nn.Parameter(torch.zeros(ch))
        # 3×3 medium
        self.off3 = nn.Sequential(nn.Conv2d(ch*2, ch, 3, padding=1), nn.GroupNorm(g, ch), nn.GELU(),
                                  nn.Conv2d(ch, 2*9, 3, padding=1))
        self.mod3 = nn.Sequential(nn.Conv2d(ch*2, ch, 3, padding=1), nn.GroupNorm(g, ch), nn.GELU(),
                                  nn.Conv2d(ch, 9, 3, padding=1))
    ...
    def forward(self, y, y_m):
        ...
        a5 = F.gelu(deform_conv2d(y_m, self.off5(cat), self.w5, self.b5, padding=2,
                                   mask=torch.sigmoid(self.mod5(cat))))
        a3 = F.gelu(deform_conv2d(y_m, self.off3(cat), self.w3, self.b3, padding=1,
                                   mask=torch.sigmoid(self.mod3(cat))))
        a1 = self.fine(cat)
...
class CLM(nn.Module):
    ...
    def __init__(self, channels, feat_dim=256, temperature=0.07, num_refs=3):
        ...
        self.ref_attn = nn.Sequential(nn.Conv2d(channels, channels//4, 1), nn.GELU(),
                                      nn.Conv2d(channels//4, 1, 1))
        self.fusion = nn.Sequential(nn.Conv2d(channels*2, channels, 3, padding=1), nn.GELU(),
...
class SimpleCLM(nn.Module):
    ...
    def __init__(self, channels, temperature=0.07, num_refs=3):
        ...
        self.off = nn.Sequential(nn.Conv2d(channels*2, channels, 3, padding=1), nn.GELU(),
    ...
    def _warp(self, feat, offset):
        B, _, H, W = feat.shape
        gy, gx = torch.meshgrid(torch.linspace(-1, 1, H, device=feat.device),
    ...
    def forward(self, y, y_refs):
        ...
        for yr in y_refs:
            ...
            attn = F.softmax(torch.bmm(query.flatten(2).transpose(1, 2),
                                       key.flatten(2)) / (C**0.5 * self.tau), dim=-1)
